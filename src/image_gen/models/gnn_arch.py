...
class Model(pl.LightningModule):
    def __init__(self, config):
        ...
        #self.lins = torch.nn.ModuleList([nn.Linear(16,4) for _ in range(self.num_layers)])

        self.norms = torch.nn.ModuleList([nn.LayerNorm(hidden_channels) for _ in range(self.num_layers)])
    ...
    def forward(self, x, t: int, cat: int):
        ...
        for idx, conv in enumerate(self.convs):
            to_add = F.gelu(conv(x_dict, {k:v[0].to(self.device) for k,v in edge_index_dict.items()})['x'])
            #print(f'%%%%%%%%%%%%%%%%% {x_dict["x"].shape} {to_add.shape}')
            x_dict['x'] = self.norms[idx](x_dict['x'] + to_add)
