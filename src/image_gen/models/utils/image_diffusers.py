...
class imageDiffuser(nn.Module):
    def __init__(self, T:int, t_start:float = 1e-4, t_end:float = 2e-2):
        ...
        self.noise_schedule= nn.Parameter(torch.linspace(t_start, t_end, T),requires_grad=False)
        ...
        self.rev_image_coefs = nn.Parameter(torch.sqrt(1 / image_schedule), requires_grad=False)
        self.rev_pred_noise_coefs = nn.Parameter((1 - image_schedule) / torch.sqrt(1 - self.image_coefs),  requires_grad=False)
