...
class Model(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        imgs_noisy = train_batch["filtered_low_pass"].to(self.device)
        cat = train_batch["encoded_sentence"].to(self.device)
        self.last_cat = torch.clone(cat[:3])
        # print('3333333333333333')
        # for i in train_batch:
        #     if hasattr(train_batch[i], 'shape'):
        #         print(f"4444444 {i} {train_batch[i].shape}")
        ...
        z = torch.randn(self.number_of_generated_batches*imgs_noisy.shape[0], self.noise_vector_len, device=self.device)
        z_cat = cat.repeat(self.number_of_generated_batches, 1)
        y_generator = (-1) * torch.ones(self.number_of_generated_batches*imgs_noisy.shape[0], device=self.device)
